                ...
                switch (item.type) {
                    case 'range':
                        if (rangesUsed < rangesCreated) {
                            ...
                            // create a new range
                            var domItem = this.createEventRange(item.content, item.groupId, item.id);
...
Timeline.prototype.createEventDot = function(content) {
  ...
  return divBox;
     ...
     *                             or HTML code.
     */ 
    Timeline.prototype.createEventRange = function(content, groupId, eventId) {
        // background box
        var divBox            = document.createElement("DIV"),
            groupClass        = ( groupId ) ? " timeline-group-" + groupId : "",
            eventClass        = ( eventId ) ? " timeline-event-" + eventId : "";

        divBox.style.position = "absolute";
        divBox.className      = "timeline-event timeline-event-range" + groupClass + eventClass;
  ...
  return sel;
    ...
    Timeline.prototype.selectItem = function(index) {
        ...
        if (this.items[index] !== undefined) {
            ...
            switch (item.type) {
                case 'range':
                    domItem.className = domItem.className + " timeline-event-selected";
                    break;
                case 'box':
    ...
    Timeline.prototype.unselectItem = function() {
        if (this.selection) {
            ...
            if (item && item.dom) {
                ...
                switch (item.type) {
                    case 'range':
                        domItem.className = domItem.className.replace(" timeline-event-selected","");
                        break;
                    case 'box':
